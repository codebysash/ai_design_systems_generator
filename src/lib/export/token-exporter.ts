...
export class TokenExporter {
  async exportTokens(
    ...
    if (options.includeDocumentation) {
      result.documentation = this.generateDocumentation(
        transformedTokens,
        options
      )
    }
  ...
  private transformTokens(
    ...
    if (options.includeSemanticTokens) {
      transformedTokens.semantic = this.generateSemanticTokens(
        tokens,
        lightTheme,
        darkTheme
      )
    ...
    if (options.transformNames !== 'camelCase') {
      transformedTokens = this.transformTokenNames(
        transformedTokens,
        options.transformNames
      )
    }
  ...
  private generateSemanticTokens(
    ...
    return {
      color: {
        text: {
          primary:
            lightTheme?.foreground?.primary || tokens.colors.neutral[900],
          secondary:
            lightTheme?.foreground?.secondary || tokens.colors.neutral[600],
          tertiary:
            lightTheme?.foreground?.tertiary || tokens.colors.neutral[400],
          inverse: lightTheme?.foreground?.inverse || tokens.colors.neutral[50],
          disabled:
            lightTheme?.foreground?.disabled || tokens.colors.neutral[300],
        },
        background: {
          primary: lightTheme?.background?.primary || tokens.colors.neutral[50],
          secondary:
            lightTheme?.background?.secondary || tokens.colors.neutral[100],
          tertiary:
            lightTheme?.background?.tertiary || tokens.colors.neutral[200],
          inverse:
            lightTheme?.background?.inverse || tokens.colors.neutral[900],
          disabled:
            lightTheme?.background?.disabled || tokens.colors.neutral[100],
        },
        border: {
          primary: lightTheme?.border?.primary || tokens.colors.neutral[200],
          secondary:
            lightTheme?.border?.secondary || tokens.colors.neutral[100],
    ...
    const categorizeTokens = (obj: any, path: string[] = []): void => {
      for (const [key, value] of Object.entries(obj)) {
        ...
        if (
          ...
          this.setNestedValue((categories as any)[category], currentPath, value)
        }
      }
    }
  ...
  private determineCategory(path: string[]): string {
    ...
    if (
      firstSegment?.includes('color') ||
      firstSegment?.includes('background') ||
      firstSegment?.includes('border')
    ) {
      return 'color'
    }
    if (
      firstSegment?.includes('font') ||
      firstSegment?.includes('text') ||
      firstSegment?.includes('typography')
    ) {
      return 'typography'
    }
    if (
      firstSegment?.includes('space') ||
      firstSegment?.includes('gap') ||
      firstSegment?.includes('margin') ||
      firstSegment?.includes('padding')
    ) {
      return 'spacing'
    }
    if (
      firstSegment?.includes('size') ||
      firstSegment?.includes('width') ||
      firstSegment?.includes('height')
    ) {
      return 'sizing'
    }
    if (
      firstSegment?.includes('animation') ||
      firstSegment?.includes('transition') ||
      firstSegment?.includes('duration')
    ) {
      return 'animation'
    }
  ...
  private async generateTokensOutput(
    tokens: any,
    options: TokenExportOptions
  ): Promise<string> {
  ...
  private generateJSONOutput(tokens: any, options: TokenExportOptions): string {
    const output = {
      ...(options.includeMetadata && {
        $metadata: this.generateInlineMetadata(options),
      }),
      ...tokens,
    }
...
${tomlString}`
  ...
  private generateStyleDictionaryOutput(
    tokens: any,
    options: TokenExportOptions
  ): string {
  ...
  private validateSpacingTokens(
    ...
    for (const [spaceName, spaceValue] of Object.entries(spacing)) {
      if (
        ...
        !this.isValidLength(spaceValue as string)
      ) {
        errors.push({
  ...
  private validateBorderRadiusTokens(
    borderRadius: any,
    errors: TokenValidationError[],
    warnings: TokenValidationWarning[]
  ): void {
    ...
    for (const [radiusName, radiusValue] of Object.entries(borderRadius)) {
      if (
        ...
        !this.isValidLength(radiusValue as string)
      ) {
        errors.push({
  ...
  private generateMetadata(
    ...
    return {
      ...
      description:
        ...
        'AI-generated design system tokens',
  ...
  private generateTokenInterfaces(
    tokens: any,
    interfaceName = 'DesignTokens'
  ): string {
